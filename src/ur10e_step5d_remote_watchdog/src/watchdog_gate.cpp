...
WatchdogGate::WatchdogGate(const WatchdogConfig &config) : config_(config) {
  if (!std::isfinite(config_.max_abs_velocity_rad_s) ||
      ...
      !std::isfinite(config_.stale_timeout_s) ||
      config_.stale_timeout_s <= 0.0) {
...
void WatchdogGate::submit(const CommandEvent &event) {
  ...
  if (event.kind == CommandEventKind::INVALID_NONFINITE ||
      ...
      !std::isfinite(event.received_at_s)) {
...
JointCommand WatchdogGate::update(const double now_s, const double period_s) {
  ...
  for (std::size_t index = 0; index < kJointCount; ++index) {
    if (std::abs(pending_command_[index] - last_output_[index]) >
        delta_limit + 1e-12) {
...
WatchdogStatus WatchdogGate::status() const noexcept { return status_; }

bool WatchdogGate::latched() const noexcept { return latched_; }
...
JointCommand WatchdogGate::zero() noexcept { return JointCommand{}; }
