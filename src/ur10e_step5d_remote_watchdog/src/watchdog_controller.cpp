...
WatchdogController::command_interface_configuration() const {
  ...
  for (const auto &joint : joint_names_) {
    configuration.names.push_back(joint + "/" +
...
WatchdogController::on_configure(const rclcpp_lifecycle::State &) {
  const auto node = get_node();
  joint_names_ = node->get_parameter("joints").as_string_array();
  if (joint_names_.size() != kJointCount ||
      std::any_of(joint_names_.begin(), joint_names_.end(),
  ...
  if (std::adjacent_find(unique_names.begin(), unique_names.end()) !=
      unique_names.end()) {
  ...
  try {
    ...
    RCLCPP_ERROR(node->get_logger(), "watchdog configuration rejected: %s",
                 error.what());
    return controller_interface::CallbackReturn::ERROR;
  }
...
controller_interface::CallbackReturn
WatchdogController::on_activate(const rclcpp_lifecycle::State &) {
...
WatchdogController::update(const rclcpp::Time &,
  ...
  published_status_.store(static_cast<std::uint8_t>(gate_->status()),
                          std::memory_order_release);
...
PLUGINLIB_EXPORT_CLASS(ur10e_step5d_remote_watchdog::WatchdogController,
