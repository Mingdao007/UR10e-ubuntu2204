...
void emulate_controller_sequence(WatchdogGate & gate, std::uint64_t & processed_sequence,
...
TEST(WatchdogGate, RepeatedSequenceIsAcceptedAsDuplicateCommand)
{
  ...
  const JointCommand expected = {
    {0.001, 0.001, 0.001, 0.001, 0.001, 0.001}};

  emulate_controller_sequence(gate, processed_sequence, valid_event(0.001, 1.000, 1));
  EXPECT_EQ(gate.update(1.001, 0.002), expected);
  EXPECT_EQ(gate.status(), WatchdogStatus::ACTIVE);

  // Same sequence should be treated as duplicate and ignored; output remains active.
  emulate_controller_sequence(gate, processed_sequence, valid_event(0.003, 1.004, 1));
  EXPECT_EQ(gate.update(1.004, 0.002), expected);
  EXPECT_EQ(gate.status(), WatchdogStatus::ACTIVE);
  EXPECT_FALSE(gate.latched());

  // Next sequence should still be accepted.
  emulate_controller_sequence(gate, processed_sequence, valid_event(0.001, 1.004, 2));
...
TEST(WatchdogGate, OutOfOrderSequenceLatchesAsCommandOverrun)
{
  ...
  emulate_controller_sequence(gate, processed_sequence, valid_event(0.001, 1.0, 1));
  EXPECT_EQ(gate.update(1.001, 0.002), expected);
  EXPECT_EQ(gate.status(), WatchdogStatus::ACTIVE);

  emulate_controller_sequence(gate, processed_sequence, valid_event(0.001, 1.002, 2));
  EXPECT_EQ(gate.update(1.004, 0.002), expected);

  // Older sequence should not be accepted and must latch COMMAND_OVERRUN.
  emulate_controller_sequence(gate, processed_sequence, valid_event(0.001, 1.003, 1));
...
TEST(WatchdogGate, LifecycleResetClearsLatchAndAcceptsFreshCommand)
{
  ...
  const JointCommand expected = {
    {0.001, 0.001, 0.001, 0.001, 0.001, 0.001}};
  gate.submit(valid_event(0.002, 1.0, 1));  // violates slew with max acceleration
  EXPECT_EQ(gate.update(1.001, 0.002), JointCommand{});
  EXPECT_TRUE(gate.latched());
  EXPECT_EQ(gate.status(), WatchdogStatus::LATCH_SLEW_LIMIT);
