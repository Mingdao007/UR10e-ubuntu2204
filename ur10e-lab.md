# UR10e Lab

This vault tracks UR10e bring-up, experiments, reports, and robot-side notes.

## Current Experiment

- [[experiments/zero_drift/20260518_initial_s00_s01/report|UR10e Zero Drift Initial S00/S01 Report]]
- [[experiments/zero_drift/20260518_initial_s00_s01/protocol|First Drift Protocol]]

## Rules

- New experiment, new git branch.
- New experiment, new English-named subfolder under `experiments/`.
- Every completed experiment must have a `report.md`.
- Keep ROS build products out of git.

## Important Paths

- Experiments: [[experiments/README]]
- Zero drift records: [[experiments/zero_drift/README]]
- ROS wrapper: `src/ur10e_bringup`
