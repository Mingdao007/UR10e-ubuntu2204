...
RUN_DIR="${RUN_ROOT}/step5a_historical_fixed_z_position_${STAMP}"
...
echo "target=historical Step5a safe-frame start, active TCP, fixed_z + 10mm air gap"
echo "position_speed_m_s=${STEP5A_POSITION_SPEED_M_S}"
...
print_readiness_failure() {
  echo "fixed-Z historical 5a positioning readiness failed; no motion was attempted."
  echo "run_dir=${RUN_DIR}"
...
for attempt in $(seq 1 "${STEP5A_DRIVER_LAUNCH_ATTEMPTS}"); do
  LAUNCH_LOG="${RUN_DIR}/historical_position_launch_attempt_${attempt}.log"
  ...
  ln -sfn "$(basename "${LAUNCH_LOG}")" "${RUN_DIR}/historical_position_launch.log"
  ...
  echo "historical fixed-Z position driver launch attempt ${attempt}/${STEP5A_DRIVER_LAUNCH_ATTEMPTS}"
  ...
  if ros2 run ur10e_example_controllers step5a_driver_readiness_check \
    ...
    --role "step5a_historical_fixed_z_position_driver_readiness_gate" \
  ...
  if (( attempt < STEP5A_DRIVER_LAUNCH_ATTEMPTS )) && readiness_failed; then
    echo "driver startup failed before fixed-Z positioning; retrying after ${STEP5A_DRIVER_RETRY_SLEEP_S}s."
...
echo "step5a historical fixed-Z position complete: ${RUN_DIR}"
