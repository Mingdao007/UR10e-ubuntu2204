...
这份报告现在按现场操作优先组织。你正在调 OnRobot 时，先不要从照片全集开始读，按下面顺序走：

1. 先看“包装盒 1-10 编号对照”，确认手里每个部件对应盒盖哪个编号，哪些已经拍到，哪些还待确认。
2. 马上要接线或跑 drift 时，看“第一次上手：不安装到 UR10e 的 bench drift 测试”。
3. 需要把这次 bench 数据推进到 TASE 准备时，看“第一阶段 Bring-Up Checklist”和“TASE 接触实验前测试清单”。
4. 要做借用归档、还原包装、查缺件或给别人交接时，再看后面的详细照片证据分组。
5. 完整 contact sheets 放在文末“Visual Overview”，作为最后的视觉索引。

## 包装盒 1-10 编号对照

盒盖 packout diagram 是本套件的 inventory index。实际图上是 `1-10`，不是 `1-9`；下面把每个编号直接对应到当前归档照片、现在的确认状态和现场用途。

![Box packout diagram](assets/thumbnails/IMG_1166.jpg)

| 盒盖编号 | 盒盖名称 | 对应照片 | 当前状态 | 用途与注意事项 |
|---:|---|---|---|---|
| 1 | Compute Box / EtherCAT Converter | `IMG_1202-1205`；核心证据 `IMG_1203` | 已确认 Compute Box 外观和端口面 | 第一次 bench bring-up 的核心盒体。接线时以 `IMG_1203` 的 `24V`、`DEVICE`、`ETHERNET`、LED、DIP switch 和固定 IP `192.168.1.1` 为准。 |
| 2 | Pen Drive | `IMG_1187-1188` | 已确认 OnRobot USB pen drive 外观 | 可能包含 URCap、manual 或示例文件；第一次 bench test 不直接安装未知 installer，先只读目录和版本。 |
| 3 | HEX-E / HEX-H Sensor | `IMG_1179-1183`；核心身份 `IMG_1182` | 已确认为 `HEX-E v2`，S/N `3010007655` | 这是 force/torque sensor 本体；所有 baseline、截图和 run metadata 都绑定这个 serial。 |
| 4 | Adapter & Screw Set with Cable Holder | `IMG_1176`、`IMG_1178`、`IMG_1184-1186` | 已看到 Adapter flange A 和若干 accessory bag，但小件功能未逐件拆分 | 后续装到 UR10e 前必须用 manual 对照 robot-side/tool-side、螺丝规格和 cable holder；不要凭照片选螺丝。 |
| 5 | Tool for Screws | 当前照片未清楚单独确认；可能在 `IMG_1184-1186` accessory bag 内 | 待确认 | 不把任何袋内小工具默认当成这个编号。拆包后需要补一张 tool close-up，并核对是否缺件。 |
| 6 | PG16 Cable Gland | `IMG_1197-1201` | 已确认 PG16 cable gland 外观 | 主要用于后续正式 cable route、strain relief 或 enclosure pass-through；临时 bench bring-up 可先不使用。 |
| 7 | 24V Universal Mount Adapter | `IMG_1173`；补充 `IMG_1174` | 已确认 24V adapter 和 safety leaflet | Compute Box 独立上电的电源。第一次使用时最后插入 `24V` 口，先观察 LED，不接 UR 控制柜。 |
| 8 | 24V Robot Power Cable (1m) | `IMG_1190` | 已看到一端 round connector、另一端 stripped wires | 高风险线缆。裸线端不得接到 24V、UR IO、控制柜或未知端口，除非 manual/pinout 已确认。 |
| 9 | UTP 0.5m | `IMG_1191` | 已确认蓝色 Ethernet/UTP cable | 用于电脑直连 Compute Box `ETHERNET` 口；电脑设置到 `192.168.1.2/24` 后 ping `192.168.1.1`。 |
| 10 | Sensor Cable (M8 - 5m) | `IMG_1194-1196`；补充 `IMG_1193`、`IMG_1195` | 已看到 coiled black cable、connector 和 label，但完整 part number 仍需补拍 | 连接 sensor / Compute Box 的主线缆候选。正式安装时要记录 cable route，做 cable strain sensitivity check。 |

这个表是“缺件与用途”的主索引。后面的详细照片分组负责解释每张图本身；如果两者冲突，以实物 label、manual 和后续拆包近照为准。
...
`IMG_1166.HEIC` 是包装盒 packout diagram，已前置到“包装盒 1-10 编号对照”。本节只保留 calibration、packing slip 和箱体规格标签这类身份/traceability 证据。
...
- 顺序/packout 调整前报告备份：`onrobot_hex_e_v2_reproduction_prep_20260518.before_order_packout_reflow.md`
- md-report-skill rollback backup: `/Users/andyl/.codex/backups/onrobot-order-packout-md-report-skill-20260519_021727/`

## Visual Overview

这是文末视觉索引，不是操作入口。三张 contact sheet 用来快速回看整批照片：第一张是文档、UR10e 现场和传感器主体；第二张是传感器细节、附件和主要线缆；第三张是线缆、PG16 cable gland 和 Compute Box。
