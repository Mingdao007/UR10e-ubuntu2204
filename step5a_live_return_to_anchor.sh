...
RUN_DIR="${RUN_ROOT}/step5a_return_to_anchor_${STAMP}"
SOURCE_RUN_DIR="${1:-}"
...
print_readiness_failure() {
  echo "return-to-anchor 5a0 failed; no return motion was attempted."
  echo "run_dir=${RUN_DIR}"
  echo "source_run_dir=${SOURCE_RUN_DIR}"
...
for attempt in $(seq 1 "${STEP5A_DRIVER_LAUNCH_ATTEMPTS}"); do
  LAUNCH_LOG="${RUN_DIR}/return_launch_attempt_${attempt}.log"
  ...
  ln -sfn "$(basename "${LAUNCH_LOG}")" "${RUN_DIR}/return_launch.log"
  ...
  echo "return driver launch attempt ${attempt}/${STEP5A_DRIVER_LAUNCH_ATTEMPTS}"
  ...
  if ros2 run ur10e_example_controllers step5a_driver_readiness_check \
    ...
    --role "step5a_return_to_anchor_driver_readiness_gate" \
  ...
  if (( attempt < STEP5A_DRIVER_LAUNCH_ATTEMPTS )) && readiness_failed; then
    echo "return driver startup failed before any motion; retrying after ${STEP5A_DRIVER_RETRY_SLEEP_S}s."
...
ros2 run ur10e_example_controllers step5a_return_to_anchor_motion \
  --execute \
...
echo "step5a return-to-anchor complete: ${RUN_DIR}"
