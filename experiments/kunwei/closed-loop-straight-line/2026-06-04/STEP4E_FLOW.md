...
Current package: `step4e_seed_normal_loop_v30`

Previous package retained for locked-normal fallback: `step4e_seed_normal_loop_v29`
...
| 12 | `25.0` | bridge + TP | Run straight XY line while maintaining normal force. v30 changes only the attitude/normal reference here: use friction-projected, gated, slew-limited filtered live normal; fall back/freeze to locked-normal behavior when the candidate is invalid. | `speedl([vx,vy,vz,wx,wy,0])`, `a=0.300 m/s^2`, `t=0.002 s`; operator default line speed `0.003 m/s`. | Linear command from force/path controller; angular `wx/wy` allowed, `wz` rejected above `0.005 rad/s`; diagnostics log raw live normal, projected candidate, filtered/control normal, candidate angle, and filter source. | End progress hold for `0.100 s`, stop register `1`. | `13` command over-limit, `12` command timeout, `10` line timeout. |
