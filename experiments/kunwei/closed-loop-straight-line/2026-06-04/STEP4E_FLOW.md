...
Current package: `step4e_seed_normal_loop_v29`
...
| 8 | `25.2` | bridge + TP | Lifted attitude correction using angular `speedl` only, after input registers `37..39` have settled to zero. | `speedl([0,0,0,wx,wy,wz])`, `a=0.300 m/s^2`, `t=0.002 s`; angular limit `0.120 rad/s`; ignore <= `3 deg`; hard stop > `30 deg`. | For v23..v29, input registers `37..39` must be exactly zero; only `40..42` may be nonzero angular command; `cmd_valid=1`. | Orientation error <= `3 deg`, then continue. | `13` if linear registers are nonzero after settle or angular command exceeds limit; `15` if > `30 deg`; `10` timeout. |
