...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2c_admittance_search30_v4base_trigger1n_v6.urp"
EXPECTED_BASENAME="step2c_admittance_search30_v4base_trigger1n_v6.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2c-admittance-search30-v4base-trigger1n-v6-operator.sh bridge
  step2c-admittance-search30-v4base-trigger1n-v6-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step2/step2c_admittance_search30_v4base_trigger1n_v6.urp
...
Motion settings in the TP program:
  ...
  early contact trigger: signed Fz <= -1.0 N in the near zone
...
ax.set_title("Step2C v6 v4base trigger1n Fz Tracking")
...
while True:
    ...
    if not snap["loaded_ok"]:
        print(snap["raw"], flush=True)
        print("[autowatch] refusing: loaded program is not the expected v6 .urp", flush=True)
        raise SystemExit(6)

    if snap["running"]:
        print("[autowatch] expected v6 program is running; starting bridge.", flush=True)
        raise SystemExit(0)
...
case "${mode}" in
  autowatch)
    cat <<'WARNING'
STEP2C admittance-search30 v6 v4base trigger1n autowatch.
...
The bridge will start automatically only after Dashboard reports that exact v6 program running.
WARNING
    ...
    STAMP="$(date +%Y%m%d_%H%M%S)"
    out_dir="${RUN_ROOT}/bridge_step2c_v6_v4base_trigger1n_autowatch_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
  ...
  bridge)
    cat <<'WARNING'
STEP2C admittance-search30 v6 v4base trigger1n lifecycle bridge.
...
WARNING
    ...
    out_dir="${RUN_ROOT}/bridge_step2c_v6_v4base_trigger1n_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
