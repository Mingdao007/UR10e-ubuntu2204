...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step4/step4c_circle_attitude_admittance_v1.urp"
EXPECTED_BASENAME="step4c_circle_attitude_admittance_v1.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step4c-circle-v1-operator.sh bridge
  step4c-circle-v1-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step4/step4c_circle_attitude_admittance_v1.urp
...
Bridge lifecycle:
  ...
  writes stage_frequency_summary.json and step4c_circle_analysis_summary.json after bridge exit
...
Motion settings in the TP program:
  ...
  compliance: signed-Fz Z admittance plus bounded wx/wy velocity attitude admittance
USAGE
}
...
PY
  ...
  while true; do
    if dashboard_snapshot >/tmp/step4_dash_snapshot.txt 2>&1; then
      ...
      if [[ "${rc}" == "10" ]]; then
        echo "[autowatch] expected Step4c program is running; starting bridge."
        return 0
      elif [[ "${rc}" == "20" || "${rc}" == "21" ]]; then
        echo "[autowatch] refusing: safety is not NORMAL or loaded program is not Step4c"
        return "${rc}"
      fi
    fi
    now="$(python3 - <<'PY'
...
postprocess_run() {
  ...
  if [[ -f "${bridge_csv}" ]]; then
    ...
    python3 "${ROOT}/tools/analyze_step2d_circle_run.py" "${bridge_csv}" --target-force-n 5 --output "${out_dir}/step4c_circle_analysis_summary.json" || true
  else
...
case "${mode}" in
  autowatch)
    cat <<'WARNING'
STEP4c circle contact v1 autowatch.
...
The bridge will start automatically only after Dashboard reports that exact Step4c program running.
WARNING
    run_bench_gate
    ensure_no_existing_bridge
    wait_for_tp_play_autowatch
    run_bridge_for_mode "${RUN_ROOT}/bridge_step4c_circle_v1_autowatch_attitude_admittance_${STAMP}" 1
    ;;
  bridge)
    cat <<'WARNING'
STEP4c circle contact v1 lifecycle bridge.
...
Type START_STEP4C_CIRCLE_V1 to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP4C_CIRCLE_V1" ]]; then
      echo "aborted"
      exit 2
    fi
    run_bench_gate
    ensure_no_existing_bridge
    run_bridge_for_mode "${RUN_ROOT}/bridge_step4c_circle_v1_attitude_admittance_${STAMP}" 0
    ;;
  *)
    usage
    exit 2
