...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step4/step4d_circle_detsearch_attitude_v1.urp"
EXPECTED_BASENAME="step4d_circle_detsearch_attitude_v1.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step4d-circle-v1-operator.sh bridge
  step4d-circle-v1-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step4/step4d_circle_detsearch_attitude_v1.urp
...
Bridge lifecycle:
  ...
  writes stage_frequency_summary.json and step4d_circle_analysis_summary.json after bridge exit

Motion settings in the TP program:
  scaffold: bounded high approach, deterministic contact search, short retract, home return
  circle: full circle from the middle-half diameter of the prior contact path
  IK: UR owns IK through speedl Cartesian twist
  search stage: 15 mm/s far, 3 mm/s near, speedl acceleration 300 mm/s^2
  contact latch: near-zone raw Fz <= -1.0 N or force norm > 1.5 N
  pre-contact: no force-admittance downward command
  line/circle stage: 5 mm/s tangent, speedl acceleration 300 mm/s^2,
    first 0.10 s at t=10 ms, then t=1 ms, runtime limit 30 s
  compliance: signed-Fz Z admittance after contact latch plus bounded wx/wy velocity attitude admittance
USAGE
}
...
PY
  ...
  while true; do
    if dashboard_snapshot >/tmp/step4_dash_snapshot.txt 2>&1; then
      ...
      if [[ "${rc}" == "10" ]]; then
        echo "[autowatch] expected Step4d program is running; starting bridge."
        return 0
      elif [[ "${rc}" == "20" || "${rc}" == "21" ]]; then
        echo "[autowatch] refusing: safety is not NORMAL or loaded program is not Step4d"
        return "${rc}"
      fi
    fi
    now="$(python3 - <<'PY'
...
postprocess_run() {
  ...
  if [[ -f "${bridge_csv}" ]]; then
    ...
    python3 "${ROOT}/tools/analyze_step2d_circle_run.py" "${bridge_csv}" --target-force-n 5 --output "${out_dir}/step4d_circle_analysis_summary.json" || true
  else
...
case "${mode}" in
  autowatch)
    cat <<'WARNING'
STEP4d circle contact v1 autowatch.
...
The bridge will start automatically only after Dashboard reports that exact Step4d program running.
WARNING
    run_bench_gate
    ensure_no_existing_bridge
    wait_for_tp_play_autowatch
    run_bridge_for_mode "${RUN_ROOT}/bridge_step4d_circle_v1_autowatch_detsearch_attitude_${STAMP}" 1
    ;;
  bridge)
    cat <<'WARNING'
STEP4d circle contact v1 lifecycle bridge.
...
Motion:
  ...
  search = 15 mm/s far, 3 mm/s near, speedl acceleration = 300 mm/s^2
  pre-contact search has no force-admittance downward command
  tangent = 5 mm/s, speedl acceleration = 300 mm/s^2
  Z force admittance = signed-Fz P/I correction after contact latch
  target force ramp = 1 N to 5 N over 2 s
  attitude = bounded wx/wy velocity admittance from filtered force/torque
...
Type START_STEP4D_CIRCLE_V1 to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP4D_CIRCLE_V1" ]]; then
      echo "aborted"
      exit 2
    fi
    run_bench_gate
    ensure_no_existing_bridge
    run_bridge_for_mode "${RUN_ROOT}/bridge_step4d_circle_v1_detsearch_attitude_${STAMP}" 0
    ;;
  *)
    usage
    exit 2
