...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2c_admittance_search15_guard20_search2ms_line1ms_alpha70_v9_v4copy_far15.urp"
EXPECTED_BASENAME="step2c_admittance_search15_guard20_search2ms_line1ms_alpha70_v9_v4copy_far15.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2c-admittance-search15-v9-v4copy-far15-operator.sh bridge
  step2c-admittance-search15-v9-v4copy-far15-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step2/step2c_admittance_search15_guard20_search2ms_line1ms_alpha70_v9_v4copy_far15.urp
...
ax.set_title("Step2C v9 v4copy far15 Fz Tracking")
...
while True:
    ...
    if not snap["loaded_ok"]:
        print(snap["raw"], flush=True)
        print("[autowatch] refusing: loaded program is not the expected v9 .urp", flush=True)
        raise SystemExit(6)

    if snap["running"]:
        print("[autowatch] expected v9 program is running; starting bridge.", flush=True)
        raise SystemExit(0)
...
case "${mode}" in
  autowatch)
    cat <<'WARNING'
STEP2C admittance-search15 v9 v4copy far15 autowatch.
...
The bridge will start automatically only after Dashboard reports that exact v9 program running.
WARNING
    ...
    STAMP="$(date +%Y%m%d_%H%M%S)"
    out_dir="${RUN_ROOT}/bridge_step2c_v9_v4copy_far15_autowatch_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
  ...
  bridge)
    cat <<'WARNING'
STEP2C admittance-search15 v9 v4copy far15 lifecycle bridge.
...
Type START_STEP2C_ADMITTANCE15_V9_V4COPY_FAR15 to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP2C_ADMITTANCE15_V9_V4COPY_FAR15" ]]; then
      echo "aborted"
      exit 2
    fi

    out_dir="${RUN_ROOT}/bridge_step2c_v9_v4copy_far15_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
