...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2c_final.urp"
EXPECTED_BASENAME="step2c_final.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2c-final-operator.sh bridge
  step2c-final-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step2/step2c_final.urp
...
ax.set_title("Step2C Final Fz Tracking")
...
while True:
    ...
    if not snap["loaded_ok"]:
        print(snap["raw"], flush=True)
        print("[autowatch] refusing: loaded program is not the expected step2c_final .urp", flush=True)
        raise SystemExit(6)

    if snap["running"]:
        print("[autowatch] expected step2c_final program is running; starting bridge.", flush=True)
        raise SystemExit(0)
...
The bridge will start automatically only after Dashboard reports that exact final program running.
WARNING
    ...
    STAMP="$(date +%Y%m%d_%H%M%S)"
    out_dir="${RUN_ROOT}/bridge_step2c_final_autowatch_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
...
Type START_STEP2C_FINAL to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP2C_FINAL" ]]; then
      echo "aborted"
      exit 2
    fi

    out_dir="${RUN_ROOT}/bridge_step2c_final_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
