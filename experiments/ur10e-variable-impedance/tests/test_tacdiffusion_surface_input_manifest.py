...
class SurfaceInputManifestTests(unittest.TestCase):
    ...
    def test_manifest_is_native_json_and_records_fail_closed_status(self):
        ...
        self.assertFalse(payload["status"]["curved_trajectory_backend_required_for_data_collection"])
        self.assertTrue(payload["status"]["unknown_surface_episode_reference_backend_ready"])
        self.assertTrue(payload["status"]["hard_tcp_tube_guard_ready"])
        self.assertTrue(payload["status"]["durable_expert_episode_artifact_writer_ready"])
        self.assertFalse(payload["status"]["durable_expert_episode_recorder_ready"])
        self.assertFalse(payload["status"]["live_bridge_recorder_integration_ready"])
        blockers = " ".join(payload["status"]["blockers"])
        self.assertNotIn("curved model integration is still required", blockers)
        self.assertIn("unknown-surface constant-Z nominal references", blockers)
        self.assertIn("84D observation and 12D expert-action", blockers)
