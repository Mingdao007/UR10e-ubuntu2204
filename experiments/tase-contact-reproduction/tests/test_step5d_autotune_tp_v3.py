...
def test_r005_direct_campaign_preserves_v1_kernel_and_has_one_motion_owner() -> None:
    rendered = v3.render_script()
    v3.validate_rendered_script(rendered)

    assert v3.PROGRAM_NAME == "step5d_strict_rnn_autotune_v3_r005"
...
def test_r005_triplet_is_exact_and_revision_is_immutable(tmp_path: Path) -> None:
    stamp = v3.source_stamp(
        datetime(2026, 7, 20, 13, 25, tzinfo=timezone(timedelta(hours=8)))
    )
    result = v3.write_triplet(tmp_path, stamp)
    basename = "step5d_strict_rnn_autotune_v3_r005"
