...
def _bridge_context(path: Path) -> BridgeStartContext:
    environment = {
        ...
        "environment": {"capture_mode": "r006_fake_transport_rehearsal"},
    }
    identity = {
...
def test_r006_ticket_binds_bridge_and_exact_campaign_without_auth_files(
    tmp_path: Path,
) -> None:
    ...
    assert observed["tp_program_id"].endswith("_r006")
    ...
    assert gate["tp_program_id"] == "step5d_strict_rnn_autotune_v3_r006"
...
def test_r006_exact_plans_materialize_machine_binding_before_runner(
    tmp_path: Path,
) -> None:
...
def test_canonical_shell_fake_transport_reaches_r006_no_arm(tmp_path: Path) -> None:
    context_path = (tmp_path / "bridge-start-context.json").resolve()
    context = _bridge_context(context_path)
