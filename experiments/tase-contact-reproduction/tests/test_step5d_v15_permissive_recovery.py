...
class Step5dV15PermissiveRecoveryTest(unittest.TestCase):
    ...
    def test_stage_table_marks_v15a_retained_after_hold_duty_live_stop(self) -> None:
        ...
        current_candidate = next(item for item in table["stages"] if item["id"] == "step5d_strict_rnn_liveprep_v23")
        ...
        self.assertIn("controller_readback_step5d_strict_rnn_liveprep_v23", current_candidate["local_delivery_evidence"]["controller_readback"])
        current = json.loads((ROOT / "config" / "current_stage.json").read_text(encoding="utf-8"))
        self.assertEqual(current["current_stage_id"], "step5d_strict_rnn_liveprep_v23")
        self.assertEqual(current["program"], "step5d_strict_rnn_liveprep_v23")
        self.assertEqual(current["bridge_profile"]["step4e_version"], "step5d_strict_rnn_liveprep_v23")
        ...
        self.assertTrue(current["evidence"]["v23_local_package_validated"])
        self.assertTrue(current["evidence"]["v23_controller_readback_verified"])
        self.assertIn("controller_readback_step5d_strict_rnn_liveprep_v23", current["evidence"]["v23_controller_readback_manifest"])
        self.assertEqual(current["evidence"]["v23_preload_gate"]["filtered_normal_load_min_n"], 7.5)
