...
class Step5dV15PermissiveRecoveryTest(unittest.TestCase):
    ...
    def test_stage_table_marks_v15a_retained_after_hold_duty_live_stop(self) -> None:
        ...
        retained_v20 = next(item for item in table["stages"] if item["id"] == "step5d_strict_rnn_liveprep_v20")
        self.assertFalse(retained_v20["active"])
        self.assertTrue(retained_v20["complete"])
        self.assertFalse(retained_v20["completion_target"])
        self.assertTrue(retained_v20["local_delivery_evidence"]["archived_to_step5d_dir"])
        self.assertIn("programs/step5/step5d/", retained_v20["local_delivery_evidence"]["local_triplet"])
        self.assertEqual(retained_v20["live_run_evidence"]["latest_stop_reason"], "signal_sigint")
        self.assertIn("incomplete live-attempt", retained_v20["live_run_evidence"]["result"])

        retained_v21 = next(item for item in table["stages"] if item["id"] == "step5d_strict_rnn_liveprep_v21")
        self.assertFalse(retained_v21["active"])
        self.assertTrue(retained_v21["complete"])
        self.assertFalse(retained_v21["completion_target"])
        self.assertTrue(retained_v21["local_delivery_evidence"]["archived_to_step5d_dir"])
        self.assertIn("programs/step5/step5d/", retained_v21["local_delivery_evidence"]["local_triplet"])
        self.assertIn("Stage25.3->25.0 register-layout hazard", retained_v21["live_run_evidence"]["root_cause_summary"])

        retained_v22 = next(item for item in table["stages"] if item["id"] == "step5d_strict_rnn_liveprep_v22")
        self.assertFalse(retained_v22["active"])
        self.assertTrue(retained_v22["complete"])
        self.assertFalse(retained_v22["completion_target"])
        self.assertTrue(retained_v22["local_delivery_evidence"]["archived_to_step5d_dir"])
        self.assertIn("programs/step5/step5d/", retained_v22["local_delivery_evidence"]["local_triplet"])
        self.assertIn("normal_force_guard", retained_v22["live_run_evidence"]["root_cause_summary"])

        retained_v23 = next(item for item in table["stages"] if item["id"] == "step5d_strict_rnn_liveprep_v23")
        self.assertFalse(retained_v23["active"])
        self.assertTrue(retained_v23["complete"])
        self.assertTrue(retained_v23["local_delivery_evidence"]["archived_to_step5d_dir"])
        self.assertIn("tcp_cage_braking_margin_exhausted", retained_v23["live_run_evidence"]["root_cause_summary"])

        current = json.loads((ROOT / "config" / "current_stage.json").read_text(encoding="utf-8"))
        current_candidate = next(item for item in table["stages"] if item["id"] == current["current_stage_id"])
        self.assertEqual(current_candidate["id"], current["program"])
        ...
        self.assertEqual(current_candidate["current_binding"]["program"], current["program"])
        self.assertEqual(current_candidate["package_delivery"]["controller_readback_manifest"], current["controller_readback_manifest"])
        ...
        if current["program"] == "step5d_strict_rnn_liveprep_v24":
            ...
            self.assertEqual(current_candidate["guard"]["line_entry_raw_sanity_min_n"], 7.0)
            self.assertEqual(current_candidate["guard"]["line_entry_raw_sanity_max_n"], 15.0)
            self.assertEqual(current_candidate["guard"]["stage25_95_qdot_clear_required_s"], 0.006)
            self.assertEqual(current_candidate["guard"]["stage25_95_qdot_clear_timeout_s"], 1.0)
            self.assertEqual(current_candidate["guard"]["stage25_95_qdot_clear_zero_tol_rad_s"], 0.0005)
            self.assertEqual(current_candidate["guard"]["stage25_post_rnn_normal_guard_hard_stop_load_n"], 25.0)
            self.assertEqual(current_candidate["guard"]["stage25_low_load_hold_timeout_s"], 0.05)
            self.assertEqual(current_candidate["guard"]["stage25_tracking_guard_opposed_dwell_s"], 0.004)
            ...
            self.assertIn("controller_readback_step5d_strict_rnn_liveprep_v24", current_candidate["local_delivery_evidence"]["controller_readback"])
            ...
            self.fail(f"unexpected current Step5d program: {current['program']}")
        ...
        self.assertIn("Stage25.3->25.0 register-layout hazard", current["evidence"]["v21_live_attempts"]["root_cause_summary"])
        self.assertTrue(current["evidence"]["v21_local_package_validated"])
        self.assertTrue(current["evidence"]["v21_controller_readback_verified"])
        self.assertEqual(
            current["evidence"]["v21_controller_readback_manifest"],
            "runs/controller_readback_step5d_strict_rnn_liveprep_v21_20260702_211049/manifest.json",
        )
        self.assertEqual(current["evidence"]["v21_preload_gate"]["filtered_normal_load_min_n"], 7.5)
        self.assertEqual(current["evidence"]["v21_preload_gate"]["filtered_normal_load_max_n"], 14.0)
        self.assertTrue(current["evidence"]["v22_local_package_validated"])
        self.assertTrue(current["evidence"]["v22_controller_readback_verified"])
        self.assertIn("controller_readback_step5d_strict_rnn_liveprep_v22", current["evidence"]["v22_controller_readback_manifest"])
        self.assertTrue(current["evidence"]["v22_retained_after_live_failure"])
        self.assertIn("normal_force_guard", current["evidence"]["v22_live_attempts"]["root_cause_summary"])
        self.assertEqual(current["evidence"]["v22_preload_gate"]["filtered_normal_load_min_n"], 7.5)
        self.assertEqual(current["evidence"]["v22_qdot_clear_barrier"]["stage"], 25.95)
        self.assertTrue(current["evidence"]["v23_retained_after_live_failure"])
        self.assertIn("tcp_cage_braking_margin_exhausted", current["evidence"]["v23_live_attempts"]["root_cause_summary"])
        self.assertTrue(current["evidence"]["v24_local_package_validated"])
        self.assertTrue(current["evidence"]["v24_controller_readback_verified"])
        self.assertIn("controller_readback_step5d_strict_rnn_liveprep_v24", current["evidence"]["v24_controller_readback_manifest"])
        self.assertEqual(current["evidence"]["v24_preload_gate"]["filtered_normal_load_min_n"], 7.5)
        self.assertEqual(current["evidence"]["v24_preload_gate"]["force_norm_stop_n"], 25.0)
        self.assertEqual(current["evidence"]["v24_qdot_clear_barrier"]["qdot_zero_tol_rad_s"], 0.0005)
        self.assertFalse(current["evidence"]["v24_low_load_policy"]["active_reacquire_solver_qdot_executed"])
        self.assertEqual(current["evidence"]["v24_post_rnn_tracking_guard"]["opposed_dwell_stop_s"], 0.004)
