...
def test_rotational_x10_profile_cross_checks_all_five_dynamics_values() -> None:
    ...
    assert execution_profile_integer_id(profile) == 844
    assert decode_execution_profile_id(844, network_mode=True) == (1.0, 2.5, 2.5)
    ...
    assert safety["bridge_normal_max_rate_rad_s"] == 1.0
    assert safety["step5d_qdot_limit_rad_s"] == 2.5
    assert safety["step5d_autotune_normal_rate_rad_s"] == 1.0
...
def test_real_parser_round_trip_classifies_every_effective_field() -> None:
    ...
    assert report["execution_profile_id"] == "nf1000-slew250-a250"
    ...
    assert effective["bridge_normal_max_rate_rad_s"] == 1.0
    assert effective["step5d_qdot_limit_rad_s"] == 2.5
    assert effective["step5d_autotune_normal_rate_rad_s"] == 1.0
