...
class Step5dAutotuneConfigTest(unittest.TestCase):
    ...
    def test_profiles_and_handshake_are_exact(self) -> None:
        ...
        self.assertEqual(
            self.payload["baseline"]["execution_profile_integer_id"], 633
        )
        ...
        self.assertTrue(profiles[5]["live_eligible"])
    ...
    def test_search_never_stops_for_budget_or_low_ei(self) -> None:
        ...
        self.assertEqual(batch["mode"], "codex_managed_hybrid_i_batches")
        self.assertEqual(batch["candidates_per_batch"], 5)
        self.assertEqual(
            batch["i_coarse_multipliers"],
            [10, 50, 100, 500, 1000],
        )
        self.assertFalse(
            batch["parameter_only_batch_update_changes_code_fingerprint"]
        )
        self.assertFalse(batch["i_axis_requires_t2_unlock"])
