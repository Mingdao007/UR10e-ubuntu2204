...
| `step5b_contact_cycloid_bayes_loop_v2` | TP Local autotune+bridge | true | true | GPU safe Bayesian supervisor | signed `_step4e_normal_load_n` | Dedicated additive Step5b autotune route. Implemented offline but inactive and live-blocked; v2 controller readback, read-only bench preflight, UR live/contact authorization, and operator canaries remain required. The ROS2 baseline and `config/current_stage.json` remain unchanged. |
