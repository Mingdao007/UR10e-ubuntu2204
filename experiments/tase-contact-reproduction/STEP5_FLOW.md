...
| `step5b_contact_cycloid_bayes_loop_v2` | TP Local autotune+bridge | true | true | GPU safe Bayesian supervisor | signed `_step4e_normal_load_n` | Dedicated additive 12 N Step5b autotune route with force-MAE-only objective and fingerprinted Step5d parameter export. Controller read-back is verified, but the route remains inactive and live-blocked; exact loaded-program preflight, active owner-policy exception, UR live/contact authorization, and one supervised 60 s baseline remain required. The ROS2 baseline and `config/current_stage.json` remain unchanged. |
