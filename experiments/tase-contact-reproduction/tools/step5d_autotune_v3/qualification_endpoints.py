...
R012_PROGRAM_ID = "step5d_strict_rnn_autotune_v3_r012"
...
class R012RuntimeIdentity:
    protocol_version: int
    digest_hi: int
    digest_lo: int
    program_id: str = R012_PROGRAM_ID
    protocol_id: str = ROLLING_PROTOCOL_ID

    def __post_init__(self) -> None:
        if self.program_id != R012_PROGRAM_ID or self.protocol_id != ROLLING_PROTOCOL_ID:
            raise EndpointSimulatorError("TP runtime identity is not the r012 rolling release")
        if self.protocol_version != 1:
            raise EndpointSimulatorError("r012 runtime protocol version must be 1")
    ...
    @classmethod
    def from_mapping(cls, payload: Mapping[str, Any]) -> "R012RuntimeIdentity":
        return cls(
            protocol_version=payload["protocol_version"],
            digest_hi=payload["digest_hi"],
            digest_lo=payload["digest_lo"],
            program_id=payload.get("program_id", R012_PROGRAM_ID),
...
@lru_cache(maxsize=1)
def default_r012_runtime_identity() -> R012RuntimeIdentity:
    """Derive the exact current r012 literals from the canonical TP generator."""
    ...
    return R012RuntimeIdentity.from_mapping(payload)
...
class QualificationEndpointSimulator:
    ...
    def __init__(
        ...
        *,
        ...
        rtde_port: int = 0,
        kunwei_port: int = 0,
        runtime_identity: R012RuntimeIdentity | Mapping[str, Any] | None = None,
        expected_program_id: str = R012_PROGRAM_ID,
        loaded_program: str = (
        ...
        if runtime_identity is None:
            self.runtime_identity = default_r012_runtime_identity()
        elif isinstance(runtime_identity, R012RuntimeIdentity):
            self.runtime_identity = runtime_identity
        else:
            self.runtime_identity = R012RuntimeIdentity.from_mapping(runtime_identity)
        if (
