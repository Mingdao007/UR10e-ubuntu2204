#!/usr/bin/env python3
"""Build the independent gentle-contact Autotune V4 r003 TP triplet."""
...
def source_stamp(now: datetime | None = None) -> str:
    ...
    return value.strftime("%Y-%m-%dT%H%MHKT_STEP5D_AUTOTUNE_V4_R003")


def build_txt(stamp: str) -> str:
    contract = load_contract()
    return f"""Step5d independent new-EOAT 5 N Autotune V4 r003
...
Remote-owner step table:
  ...
  U2 Do not Load/Play r003 until the new EOAT is installed, three governed
     5 N baselines close,
...
def validate_triplet(
    ...
    if failed:
        raise ValueError(f"V4 r003 triplet validation failed: {failed}")
    return checks
...
def numeric_sanity(script: str) -> dict[str, Any]:
    checks = {
        ...
        "search_base_z_only": (
            f"speedl([0.0, 0.0, -{GENTLE_CONTACT_SPEED_M_S:.9f}, 0.0, 0.0, 0.0]"
            in script
        ),
    ...
    if not all(checks.values()):
        raise ValueError(f"V4 r003 numeric sanity failed: {checks}")
    return {
...
def write_triplet(output_dir: Path, stamp: str) -> dict[str, Any]:
    ...
    if any(path.exists() or path.is_symlink() for path in (*paths.values(), numeric_path, manifest_path)):
        raise FileExistsError("V4 r003 is immutable; existing output must not be overwritten")
    ...
    manifest = {
        ...
        "immutable": True,
        "builder": "tools/build_step5d_autotune_v4_r003.py",
