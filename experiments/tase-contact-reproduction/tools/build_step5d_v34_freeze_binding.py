#!/usr/bin/env python3
"""Build the recomputable v34 source/package/read-back/timing freeze binding."""
...
SOURCE_FILES = (
    ...
    "tools/verify_step5d_contact_v34.py",
    "tools/validate_step5d_v34.py",
    ...
    "scripts/step5d-strict-rnn-contact-v34.sh",
    "scripts/bridge-line-operator.sh",
    "tests/test_step5d_v34_scheduler_acceleration.py",
)
ARTIFACTS = (
    "config/step5d_v34_offline_validation.json",
    "config/step5d_v34_live_path_timing.json",
...
def build_payload(root: Path = ROOT) -> dict[str, Any]:
    ...
    if not isinstance(candidate, dict) or not isinstance(candidate.get("package"), dict):
        raise RuntimeError("missing v34_candidate.package ledger")
    package = candidate["package"]
    manifest_rel = package.get("controller_readback_manifest")
    if not isinstance(manifest_rel, str) or not manifest_rel:
        raise RuntimeError("missing v34 fresh read-back manifest")
    stage_contract = {
        key: row[key]
        for key in (
            "duration_s",
    ...
    payload: dict[str, Any] = {
        "schema": "step5d_v34_freeze_binding_v1",
        "program": PROGRAM,
