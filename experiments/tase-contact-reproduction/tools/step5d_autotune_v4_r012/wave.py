...
class WaveQualificationError(R012ValueError):
...
class WaveRunTrace:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(value, str) and value for value in (self.campaign_id, self.attempt_id, self.dispatch_id, self.schedule_id, self.execution_id, self.condition_id, self.trace_id)):
            raise WaveQualificationError("Wave run readable identity is invalid")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "WaveRunTrace":
        required = {"run_id", "matched_key", "repeat_index", "samples", "campaign_id", "attempt_id", "dispatch_id", "schedule_id", "execution_id", "condition_id", "trace_id"}
...
class WaveQualificationReceipt:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(value, str) and value for value in (self.baseline_campaign_id, self.candidate_campaign_id, self.baseline_schedule_id, self.candidate_schedule_id, self.baseline_execution_id, self.candidate_execution_id, self.condition_id)) or not all(isinstance(value, str) and value for value in self.trace_ids):
            raise WaveQualificationError("Wave receipt readable identity is invalid")
    ...
    def as_dict(self) -> dict[str, Any]:
        return {"schema": self.schema, "qualified": self.qualified, "baseline_campaign_id": self.baseline_campaign_id, "candidate_campaign_id": self.candidate_campaign_id, "baseline_schedule_id": self.baseline_schedule_id, "candidate_schedule_id": self.candidate_schedule_id, "baseline_execution_id": self.baseline_execution_id, "candidate_execution_id": self.candidate_execution_id, "condition_id": self.condition_id, "trace_ids": list(self.trace_ids), "baseline_run_ids": list(self.baseline_run_ids), "candidate_run_ids": list(self.candidate_run_ids), "baseline_metrics": [metric.as_dict() for metric in self.baseline_metrics], "candidate_metrics": [metric.as_dict() for metric in self.candidate_metrics], "gates": json_tree(self.gates), "rejection_reasons": list(self.rejection_reasons), "wave_is_bo_variable": self.wave_is_bo_variable, "trainable": self.trainable}
...
def qualify_manual_wave(
    ...
    baseline_campaign_id: str, candidate_campaign_id: str,
    baseline_schedule_id: str, candidate_schedule_id: str,
    baseline_execution_id: str, candidate_execution_id: str,
    condition_id: str, baseline_schedule: ManualWaveSchedule | Mapping[str, Any] | None = None,
    ...
    base_sched = baseline_schedule_id or str(base_schedule.identity)
    cand_sched = candidate_schedule_id or str(cand_schedule.identity)
    if not all(isinstance(value, str) and value for value in (baseline_campaign_id, candidate_campaign_id, baseline_execution_id, candidate_execution_id, condition_id)):
        raise WaveQualificationError("Wave qualification readable identity is invalid")
    ...
    for side, rows, campaign, schedule_id, execution in (("baseline", baseline, baseline_campaign_id, base_sched, baseline_execution_id), ("candidate", candidates, candidate_campaign_id, cand_sched, candidate_execution_id)):
        ...
        for run in rows:
            if run.campaign_id != campaign: reasons.append(f"campaign:{run.run_id}")
            if run.schedule_id != schedule_id: reasons.append(f"schedule:{run.run_id}")
            if run.execution_id != execution: reasons.append(f"execution:{run.run_id}")
            if run.condition_id != condition_id: reasons.append(f"condition:{run.run_id}")
    ...
    return WaveQualificationReceipt(not reasons, baseline_campaign_id, candidate_campaign_id, base_sched, cand_sched, baseline_execution_id, candidate_execution_id, condition_id, tuple(run.trace_id for run in all_runs), tuple(run.run_id for run in baseline), tuple(run.run_id for run in candidates), base_metrics, cand_metrics, gates, tuple(dict.fromkeys(reasons)))
