...
def build(receipt,output):
    ...
    if recovery is not None:
        ...
        text=first+'\n# BOUNDED_RECOVERY: observed start to approved Home; translation<=3mm turn<=20mrad; a=0.01 v=0.002\n'+rest
...
'''
    ...
    if recovery is not None:text=text.replace('a=0.05m/s2 v=0.01m/s','a=0.01m/s2 v=0.002m/s')
    ...
    info={'basename':BASENAME,'stamp':stamp,'controller_directory':CONTROLLER_DIR,'home_pose':target.tolist(),
          ...
          'numeric_sanity':{'max_transfer_distance_m':float(np.linalg.norm(target[:3]-observed[:3])),
                            'speed_m_s':.002 if recovery is not None else .01,'acceleration_m_s2':.01 if recovery is not None else .05,'initial_position_tolerance_m':.002,
                            'contact':False,'force_control':False,'joint_path_check':'required separately'},'live_executed':False}
