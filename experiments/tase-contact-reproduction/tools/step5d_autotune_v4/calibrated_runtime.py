...
from .path_controller import derive_force_terms
...
class V4CalibratedRuntime:
    ...
    def __init__(self, contract: V4Contract, candidate: V4Candidate) -> None:
    ...
    def command(
        ...
        *,
        ...
        lower = np.maximum(self.model.model.lowerPositionLimit - q, -0.15)
        upper = np.minimum(self.model.model.upperPositionLimit - q, 0.15)
    ...
    def desired_twist(
        ...
        *,
        ...
        if mode == "baseline":
            ...
            linear[:2] = 0.0
            angular[:] = 0.0
        linear[2] = float(np.clip(linear[2], -0.00035, 0.00035))
        tangential_norm = float(np.linalg.norm(linear[:2]))
        if tangential_norm > 0.00035:
            linear[:2] *= 0.00035 / tangential_norm
        linear_norm = float(np.linalg.norm(linear))
        if linear_norm > 0.0005:
            linear *= 0.0005 / linear_norm
        angular_norm = float(np.linalg.norm(angular))
        if angular_norm > 0.05:
            angular *= 0.05 / angular_norm
