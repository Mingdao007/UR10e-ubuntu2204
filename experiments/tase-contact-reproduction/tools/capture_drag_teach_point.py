#!/usr/bin/env python3
"""Capture one manually positioned TCP point for drag-teach calibration.

The operator moves the robot manually on the teach pendant / robot side, then
runs this script to snapshot the current RTDE pose. This script only reads
Dashboard and RTDE output state. It does not enable freedrive, send URScript,
write RTDE inputs, load/start programs, call zero_ftsensor(), or command robot
motion.
"""
...
def default_session_dir() -> Path:
    if SESSION_HINT.exists():
        ...
        if hinted.exists():
            return hinted
    return RUN_ROOT / f"drag_teach_points_{now_stamp()}"
...
def capture(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({"ok": True, "points_json": str(points_path), "captured": point}, indent=2, sort_keys=True))
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("label", choices=["start", "end", "mid"], help="Point label to capture.")
...
if __name__ == "__main__":
    try:
        ...
        print(f"point capture blocked: {exc}", file=sys.stderr)
        raise SystemExit(2)
