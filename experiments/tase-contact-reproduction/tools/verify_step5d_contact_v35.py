#!/usr/bin/env python3
"""Deterministically verify the Step5d v35 TP/runtime contract."""
...
def verify(root: Path) -> dict[str, object]:
    ...
    if missing:
        return {"schema": "step5d_contact_v35_verification_v1", "ok": False, "failed": [f"missing: {missing}"]}
    ...
    checks = {
        ...
        "quota-safe scheduler contract": scheduler["launch_policy"] == "SCHED_OTHER/0"
        and scheduler["control_thread_policy"] == "SCHED_OTHER/0"
        and scheduler["helper_thread_policy"] == "SCHED_OTHER/0"
        and scheduler["readiness_requires_no_sched_fifo_threads"] is True
        ...
        "documentation identifies v35 only": PROGRAM in txt and "0.100 rad/s^2" in txt and "v31" not in txt,
    }
    failed = [name for name, passed in checks.items() if not passed]
    return {
        "schema": "step5d_contact_v35_verification_v1",
        "ok": not failed,
        "failed": failed,
        "checks": checks,
