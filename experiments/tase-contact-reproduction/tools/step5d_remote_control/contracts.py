...
DEFAULT_RELEASE_PATH = (
    EXPERIMENT_ROOT / "config/step5d_remote/remote_prep_v2.json"
)
RELEASE_SCHEMA = "step5d.remote-control/release-v2"
RELEASE_STAGE_ID = "step5d_strict_rnn_autotune_v3_remote_prep_v2"
...
REMOTE_EXECUTION_UID_SCHEMA = "step5d.remote-control/execution-uid-v1"
REMOTE_EXECUTION_UID_VERSION = 1
...
def _strict_bool(name: str, value: Any) -> bool:
    if not isinstance(value, bool):
        raise RemoteControlError(f"{name} must be boolean")
    return value
...
class RemoteParameterProjection:
    update_rate_hz: float
    qdot_limit_rad_s: float
    max_acceleration_rad_s2: float
    host_slew_rad_s2: float
    actuator_acceleration_rad_s2: float
    watchdog_stale_timeout_s: float
    rnn_epsilon: float
    rnn_sigr_exponent_r: float
    rnn_inner_iterations: int
    rnn_backend: str
    ...
    def as_payload(self) -> dict[str, Any]:
        return {
            ...
            "host_slew_rad_s2": self.host_slew_rad_s2,
            "actuator_acceleration_rad_s2": self.actuator_acceleration_rad_s2,
            "watchdog_stale_timeout_s": self.watchdog_stale_timeout_s,
            "rnn_epsilon": self.rnn_epsilon,
            "rnn_sigr_exponent_r": self.rnn_sigr_exponent_r,
            "rnn_inner_iterations": self.rnn_inner_iterations,
            "rnn_backend": self.rnn_backend,
        }

    @classmethod
    def from_release(cls, source: Mapping[str, Any]) -> "RemoteParameterProjection":
        required_fields = {
            "update_rate_hz",
            "qdot_limit_rad_s",
        ...
        if missing:
            raise RemoteControlError(
                f"Remote control parameters differ: missing={sorted(missing)}"
            )
        expected = {
            "update_rate_hz": float(source["update_rate_hz"]),
            "qdot_limit_rad_s": float(source["qdot_limit_rad_s"]),
            "max_acceleration_rad_s2": float(source["max_acceleration_rad_s2"]),
            "host_slew_rad_s2": float(source["host_slew_rad_s2"]),
            "actuator_acceleration_rad_s2": float(
                source["actuator_acceleration_rad_s2"]
            ),
            "watchdog_stale_timeout_s": float(source["watchdog_stale_timeout_s"]),
            "rnn_epsilon": float(source["rnn_epsilon"]),
            "rnn_sigr_exponent_r": float(source["rnn_sigr_exponent_r"]),
            "rnn_inner_iterations": int(source["rnn_inner_iterations"]),
            "rnn_backend": _strict_string("rnn_backend", source["rnn_backend"]),
        }
        unexpected = set(source) - set(expected)
        if unexpected:
            raise RemoteControlError(
                f"Remote control parameters differ: unexpected={sorted(unexpected)}"
            )
        if expected["rnn_inner_iterations"] < 1:
            raise RemoteControlError("V3 RNN inner iterations must be positive")
        return cls(
            update_rate_hz=expected["update_rate_hz"],
            qdot_limit_rad_s=expected["qdot_limit_rad_s"],
            max_acceleration_rad_s2=expected["max_acceleration_rad_s2"],
            host_slew_rad_s2=expected["host_slew_rad_s2"],
            actuator_acceleration_rad_s2=expected["actuator_acceleration_rad_s2"],
            watchdog_stale_timeout_s=expected["watchdog_stale_timeout_s"],
            rnn_epsilon=expected["rnn_epsilon"],
            rnn_sigr_exponent_r=expected["rnn_sigr_exponent_r"],
            rnn_inner_iterations=expected["rnn_inner_iterations"],
            rnn_backend=expected["rnn_backend"],
        )


@dataclass(frozen=True)
class RemoteExecutionUid:
    schema: str
    version: int
    source_transport_candidate_uid: str
    transport_id: str
    release_sha256: str
    ...
    def from_source(
        ...
        *,
        ...
        source_transport_candidate_uid = _strict_string(
            "source_transport_candidate_uid", source_transport_candidate_uid
        )
        transport_id = _strict_string("transport_id", transport_id)
        release_sha = _strict_sha("release_sha256", release_sha256)
        controller_config_sha = _strict_sha("controller_config_sha256", controller_config_sha256)
        binding = {
            "schema": REMOTE_EXECUTION_UID_SCHEMA,
            "version": REMOTE_EXECUTION_UID_VERSION,
            "source_transport_candidate_uid": source_transport_candidate_uid,
        ...
        return cls(
            ...
            release_sha256=release_sha,
            controller_config_sha256=controller_config_sha,
            execution_uid=canonical_sha256(binding),
        )
...
class RemoteRelease:
    ...
    @property
    def offline_build_only(self) -> bool:
        return bool(self.document["offline_build_only"])
...
def _derive_control_parameters(
    ...
    parameters = {
        ...
        "rnn_sigr_exponent_r": _positive_float(rows, "--step5d-sigr-exponent-r"),
    ...
    expected_bindings = {
        ...
        "qdot_limit_rad_s": effective["safety_invariant"]["step5d_qdot_limit_rad_s"],
...
def _materialize_controller_config(
    release: Mapping[str, Any], parameters: Mapping[str, Any]
) -> dict[str, Any]:
    invariants = release["transport_invariants"]
    return {
        "controller_manager": {
            "ros__parameters": {
...
def load_remote_release(
    ...
    *,
    ...
    if not isinstance(document["live_authorized"], bool):
        raise RemoteControlError("Remote release live_authorized must be boolean")
    if not isinstance(document["offline_build_only"], bool):
        raise RemoteControlError("Remote release offline_build_only must be boolean")
    ...
    controller_config = _materialize_controller_config(document, parameters)
    return RemoteRelease(
        path=path.resolve(),
        experiment_root=root,
        document=document,
        release_sha256=canonical_sha256(document),
        controller_config_sha256=canonical_sha256(controller_config),
        contract=contract,
...
def materialize_controller_config(release: RemoteRelease) -> dict[str, Any]:
    return _materialize_controller_config(release.document, release.control_parameters)
...
class PreparedControlTrial:
    ...
    transport_id: str
    execution_uid: str
    remote_parameter_projection: Mapping[str, Any]
    ...
    def payload(self) -> dict[str, Any]:
        return {
            ...
            "remote_parameter_projection": dict(self.remote_parameter_projection),
        }
...
def prepare_control_trial(
    ...
    if occurrence_uid != expected_occurrence_uid:
        raise RemoteControlError("Remote occurrence UID differs from its v3 materials")
    if transport_candidate_uid != expected_transport_uid:
        raise RemoteControlError(
            "Remote source transport UID differs from its v3 materials"
        )
    projection = RemoteParameterProjection.from_release(release.control_parameters)
    execution_uid = RemoteExecutionUid.from_source(
        source_transport_candidate_uid=str(transport_candidate_uid),
        transport_id=release.transport_id,
        release_sha256=release.release_sha256,
        controller_config_sha256=release.controller_config_sha256,
    )
    return PreparedControlTrial(
        ...
        release_sha256=release.release_sha256,
        transport_id=release.transport_id,
        execution_uid=str(execution_uid.uid),
        remote_parameter_projection=projection.as_payload(),
    )
...
def load_prepared_control_trial(
    ...
    if str(prepared.execution_uid) != str(envelope["execution_uid"]):
        raise RemoteControlError("Remote execution uid differs")
    return prepared
...
def validate_receipt(
    ...
    *,
    ...
    if execution_mode == "offline_replay" and not release.offline_build_only:
        raise RemoteControlError("this Remote release is not offline-only")
    exact_matches = {
...
def import_result(
    ...
    *,
    ...
    return {
        ...
        "execution_uid": receipt.execution_uid,
        "transport_id": receipt.transport_id,
        "command_zero_confirmed": receipt.command_zero_confirmed,
        "controller_inactive_confirmed": receipt.controller_inactive_confirmed,
    }
