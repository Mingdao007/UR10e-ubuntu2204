#!/usr/bin/env python3
"""Build the immutable independent Kunwei 1 N force-search canary r007 TP triplet."""
...
from step5d_force_search_canary_r007 import (
    CONTROLLER_DIR,
    PROGRAM_NAME,
    load_contract,
    render_script,
...
def source_stamp(now: datetime | None = None) -> str:
    ...
    return value.strftime("%Y-%m-%dT%H%MHKT_STEP5D_FORCE_SEARCH_CANARY_R007")
...
def build_txt(contract, stamp: str) -> str:
    eoat = load_new_eoat()
    return f"""Step5d independent Kunwei 1 N force-search canary r007
...
Program-side step table:
  ...
  P7 RETRACT: not used; no retract in this contract.
...
User-side / Remote-owner step table:
  ...
  U2 Start the r007 register live writer and wait for its ready receipt.
...
def validate_triplet(
    ...
    "base_z_only": script.count("speedl(") == 1
    and "speedl([0.0, 0.0, -" in script
    and _f(contract.search_speed_m_s) in script,
    "search_speed_exact": f"speedl([0.0, 0.0, -{_f(contract.search_speed_m_s)}" in script,
    "search_stop_exact": script.count(
        f"stopl({_f(contract.search_stopl_acceleration_m_s2)})"
    )
    == 1,
        ...
        "positive_contact_not_absolute": (
            "normal_load >= 0.500000000" in script
        ...
        > script.rindex("stopl("),
        "no_retract_code": "retract_start_z" not in script
        and "retract_travel_m" not in script
        and "codex_echo(14.0, reason" not in script
        and "stage14" not in script,
        "no_positive_z_speedl": "speedl([0.0, 0.0, 0." not in script,
    ...
    if failed:
        raise ValueError(f"r007 triplet validation failed: {failed}")
    return checks


def _numeric_sanity(script: str, contract) -> dict[str, Any]:
    ...
    checks = {
        ...
        "search_speed_exact_0p2_mm_s": contract.search_speed_m_s == 0.0002,
        "search_accel_exact_0p005_m_s2": contract.search_acceleration_m_s2 == 0.005,
        ...
        "no_retract": contract.retract_distance_m == 0.0,
        "contact_positive": "normal_load >= 0.500000000" in script,
        "contact_force": "force_norm >= 0.700000000" in script,
    }
    if not all(checks.values()):
        raise ValueError(f"r007 numeric sanity failed: {checks}")
    return {
...
def _manifest(stamp: str, digests: dict[str, str], numeric_sha: str) -> dict[str, Any]:
    ...
    return {
        ...
        "immutable": True,
        "builder": "tools/build_step5d_force_search_canary_r007.py",
...
def write_triplet(output_dir: Path, stamp: str) -> dict[str, Any]:
    ...
    if collisions:
        raise FileExistsError(
            "r007 is immutable; existing output must not be overwritten: "
            + ", ".join(collisions)
        )
