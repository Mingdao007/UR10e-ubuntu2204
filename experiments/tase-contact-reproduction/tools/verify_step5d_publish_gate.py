...
def verify_stage_table_current(root: Path, current_program: str) -> None:
    ...
    for retained_program, root_cause_substring in RETAINED_FAILURE_PROGRAMS.items():
        ...
        if runs and current_program != retained_program:
            retained_row = next((item for item in table.get("stages", []) if item.get("id") == retained_program), None)
            if retained_row is None:
                fail(f"{retained_program} live-run evidence exists but the stage table row is missing")
            if retained_row.get("active") is True:
                fail(f"{retained_program} live-run evidence exists but the stage table row is still active")
            retained_delivery = retained_row.get("local_delivery_evidence", {})
            if retained_delivery.get("archived_to_step5d_dir") is not True:
                fail(f"{retained_program} is not archived under programs/step5/step5d")
            retained_evidence = retained_row.get("live_run_evidence") or {}
            root_cause = str(retained_evidence.get("root_cause_summary") or "")
            if root_cause_substring not in root_cause:
                fail(f"{retained_program} retained evidence does not record expected root cause: {root_cause_substring}")
