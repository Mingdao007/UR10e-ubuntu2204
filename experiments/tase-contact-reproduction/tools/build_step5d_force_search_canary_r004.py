...
BLOCKED_MARKER = (
    OUTPUT_DIR / "step5d_force_search_canary_r004.BLOCKED.json"
)
...
def source_stamp(now: datetime | None = None) -> str:
    ...
    return value.strftime("%Y-%m-%dT%H%MHKT_STEP5D_FORCE_SEARCH_CANARY_R004")
...
Unsafe-stop recovery:
  Stop reasons 2/3/4/5/6/7/8/10/15/16 do not retract automatically.
...
def write_triplet(output_dir: Path, stamp: str) -> dict[str, Any]:
    del output_dir, stamp
    raise RuntimeError(
        "force-search r004 has a forbidden UR built-in force dependency; "
