...
physical wire echoes use the readable (21, 618001) pair.
"""
...
def load_live_contract_document(path: Path | str | None = None) -> dict[str, Any]:
    ...
    if int(payload.get("runtime_revision")) != RUNTIME_REVISION:
        raise YieldLiveContractError("native live contract revision is not 21")
    ...
    if identity != READABLE_RUNTIME_IDENTITY:
        raise YieldLiveContractError("native readable runtime identity is not (21, 618001)")
