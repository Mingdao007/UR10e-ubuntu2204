#!/usr/bin/env python3
"""Deterministically verify the Step5d v32 stage-aware contact package."""
...
def verify(root: Path) -> dict[str, object]:
    ...
    checks = {
        ...
        "joint marker internal to Stage25": "local joint_layout_code = 524.000" in script
        and "marker 524 is internal to the Stage25 joint packet" in script
        and "if cmd_valid < 0.5 or not joint_layout_ok" in script,
        "header strict joint executor": "layout-524-only speedj" in header and "multi-layout" not in header,
    ...
    return {
        "schema": "step5d_contact_v32_verification_v1",
        "program": PROGRAM,
        "ok": not failed,
        "failed": failed,
        "checks": checks,
