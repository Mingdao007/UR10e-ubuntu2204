"""Round-10 read-only extractor: one listed training artifact -> compact npz.

Reads ONLY artifacts listed in report/yield-training-report-final-v1/artifact_digest_manifest.csv
(paths relative to runs/yield-fair-training-v1), verifies the file sha256 against the manifest
before parsing, and writes /tmp/yfp10/<attempt_id>.npz plus <attempt_id>.meta.json.
One artifact at a time; no artifact is modified. Array set is identical to the round-8
extractor (extract_rows_v8.py, main-verified) so results are directly comparable.
Rows are on the PATH clock; records aligned to rows are the records whose reference phase is 'path'.
"""
...
MANIFEST = ROOT / 'report/yield-training-report-final-v1/artifact_digest_manifest.csv'
CAMPAIGN = ROOT / 'runs/yield-fair-training-v1'
OUT = Path('/tmp/yfp10')
...
def extract(entry):
    aid = entry['attempt_id']; path = CAMPAIGN / entry['path']
    ...
    if int(entry['bytes']) != path.stat().st_size:
        raise SystemExit(f'{aid}: size mismatch')
    ...
    out['entry_obs_time_s'] = np.asarray([rec['observation']['time_s'] for rec in entry_records], dtype=float)
    out['entry_true_normal_load_n'] = np.asarray([rec['result'].get('true_normal_load_n', np.nan) if isinstance(rec.get('result'), dict) else np.nan for rec in entry_records], dtype=float)
    ...
    meta['_extract'] = {'artifact_path': str(path), 'artifact_sha256': digest, 'bytes': path.stat().st_size,
                        'manifest': str(MANIFEST), 'n_rows': len(rows), 'n_records': len(records),
                        'n_entry_records': len(entry_records), 'seconds': round(time.time() - t0, 1)}
    (OUT / f'{aid}.meta.json').write_text(json.dumps(meta, indent=1, sort_keys=True))
    print(f'{aid}: ok sha={digest[:12]} rows={len(rows)} records={len(records)} entry={len(entry_records)} {time.time()-t0:.1f}s', flush=True)


def main(argv):
    wanted = argv[1:]
    entries = {e['attempt_id']: e for e in csv.DictReader(MANIFEST.open())}
    OUT.mkdir(parents=True, exist_ok=True)
    for aid in wanted:
        entry = entries[aid]  # KeyError if not listed in the final manifest
        if entry['status'] != 'complete' or entry['parsed'] != 'True':
            raise SystemExit(f'{aid}: not complete/parsed in manifest')
        if (OUT / f'{aid}.npz').exists() and (OUT / f'{aid}.meta.json').exists():
            print(f'{aid}: cached', flush=True); continue
        extract(entry)
