...
## Five real PATH units and collector retirement repair

- `tase-resident-infra-20260922165226Z` completed five actual 60 s PATHs and a real preparation refresh. Formal MAEs: 1.935749, 1.826494, 1.679940, 1.811412, 1.874860 N. Units 1--4 sealed normally; unit 5 retained complete collector evidence but failed the following observer freshness check. No sixth ARM. Automatic joint Home and Dashboard STOPPED succeeded. Infrastructure acceptance remains failed; two ordinary reuse turnarounds had median 82.675825 s.
- At unit 5, terminal worker exit was monotonic 60096.495576 and path return 60096.565220, about 69.64 ms later. The retiring collector predicate required both `_samples` and `_bins`; the real PathEvidenceCollector has `_path_samples`, so cleanup was skipped. Parent FIFO destruction delayed observer execution; its last sample at 60096.481040 became older than 80 ms at the next check.
- Retire the actual typed PATH/qualification/timing collectors, including nested list/set buffers, in serviced chunks. Service mode remains enabled during retirement so new raw observations go into service segments rather than buffers being discarded. Prior round-boundary continuity is not claimed without packet-level reconciliation; PATH evidence remains independently sealed.
- Focused lifecycle/observer regression: 24 passed. Read-only real-controller probe with real collector layout and three ~40 MB finalizations passed, no inputs or motion commands; maximum service gap 42.265 ms including fixture construction. Probe: `/tmp/tase-resident-repair/live_readonly_service_probe_retirement.py`, observations in `readonly-service-probe-04`.
