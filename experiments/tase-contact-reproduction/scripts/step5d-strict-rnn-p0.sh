...
P0_VERIFIER="${STEP5D_P0_VERIFIER_OVERRIDE:-${ROOT}/tools/verify_step5d_no_contact_p0.py}"
STAGE_ENV_EXPORTER="${ROOT}/tools/export_stage_env.py"
P0_PROFILE="${STEP5D_P0_PROFILE_OVERRIDE:-step5d_strict_rnn_no_contact_p0_v7}"
P0_CAPTURE_KEY="${STEP5D_P0_CAPTURE_KEY_OVERRIDE:-no_contact_p0_capture}"
P0_CONFIRM_TOKEN="${STEP5D_P0_CONFIRM_TOKEN_OVERRIDE:-LIVE STEP5D STRICT RNN NO CONTACT P0}"
P0_PHASE_S="${STEP5D_P0_PHASE_S:-}"
...
Boundary:
  ...
  - capture-bridge starts the dedicated no-contact P0 bridge before Teach
    Pendant Play after STEP5D_P0_CONFIRM is set exactly. Open the exact bound
...
p0_table_preflight() {
  python3 - "${ROOT}/config/current_stage.json" "${ROOT}/config/step5_stage_table.json" "${P0_PROFILE}" "${P0_CAPTURE_KEY}" <<'PY' || exit 24
import json
import sys
from pathlib import Path
from pathlib import PurePosixPath

current_path, table_path, profile, capture_key = sys.argv[1:5]
...
if capture.get("profile") != profile:
    raise SystemExit(
        ...
        f"next: stop/reopen exact {profile}, rerun capture-bridge, then press TP Play after '[operator] P0 bridge armed: press TP Play now'"
    )
...
case "$1" in
  ...
  capture-ready)
    ...
    echo "Open that exact ${P0_PROFILE} program on the Teach Pendant and keep it STOPPED."
...
PY
    ...
    if [[ -n "${P0_PHASE_S}" ]]; then
      export STEP5D_STOP_REGISTER_CANARY_S="${P0_PHASE_S}"
    fi
    ...
    verifier_args=("${run_dir}")
    if [[ -n "${P0_PHASE_S}" ]]; then
      verifier_args+=(--phase-s "${P0_PHASE_S}")
    fi
