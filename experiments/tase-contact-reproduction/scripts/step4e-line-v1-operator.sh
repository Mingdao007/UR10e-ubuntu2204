...
if [[ "${STEP4E_VERSION}" == "p0_geo_v1" ]]; then
  ...
  SEARCH_DESCRIPTION="Step5b contact cycloid baseline v2: same Step5 table contact cycloid reference, but after first-contact latch it skips the lift/25.2 cycle when bridge orientation_error is already <=4deg; bridge defaults target 15N, Kp 0.0010, Ki 0.00001, damping 8.5, and filtered-live normal alpha 0.55 through the Step5b wrapper"
