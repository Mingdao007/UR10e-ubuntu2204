...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2d_circle_full_paper_attitude_v1.urp"
EXPECTED_BASENAME="step2d_circle_full_paper_attitude_v1.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2d-circle-v1-operator.sh bridge
  step2d-circle-v1-operator.sh autowatch
...
Bridge lifecycle:
  ...
  writes stage_frequency_summary.json and step2d_circle_analysis_summary.json after bridge exit
...
Motion settings in the TP program:
  ...
  line/circle stage: 5 mm/s tangent, first 0.10 s at t=10 ms, then t=1 ms
...
PY
  ...
  while true; do
    if dashboard_snapshot >/tmp/step2d_dash_snapshot.txt 2>&1; then
      true
    else
      rc="$?"
      cat /tmp/step2d_dash_snapshot.txt || true
      if [[ "${rc}" == "10" ]]; then
        echo "[autowatch] expected Step2D program is running; starting bridge."
        return 0
      elif [[ "${rc}" == "20" || "${rc}" == "21" ]]; then
        echo "[autowatch] refusing: safety is not NORMAL or loaded program is not Step2D"
        return "${rc}"
      fi
    fi
    now="$(python3 - <<'PY'
...
postprocess_run() {
  ...
  if [[ -f "${bridge_csv}" ]]; then
    ...
    python3 "${ROOT}/tools/analyze_step2d_circle_run.py" "${bridge_csv}" --target-force-n 5 --output "${out_dir}/step2d_circle_analysis_summary.json" || true
  else
...
The bridge will start automatically only after Dashboard reports that exact Step2D program running.
WARNING
    run_bench_gate
    ensure_no_existing_bridge
    wait_for_tp_play_autowatch
    run_bridge_for_mode "${RUN_ROOT}/bridge_step2d_circle_v1_autowatch_full_paper_attitude_${STAMP}" 1
    ;;
  bridge)
    cat <<'WARNING'
STEP2D circle v1 lifecycle bridge.
...
Type START_STEP2D_CIRCLE_V1 to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP2D_CIRCLE_V1" ]]; then
      echo "aborted"
      exit 2
    fi
    run_bench_gate
    ensure_no_existing_bridge
    run_bridge_for_mode "${RUN_ROOT}/bridge_step2d_circle_v1_full_paper_attitude_${STAMP}" 0
    ;;
  *)
    usage
    exit 2
