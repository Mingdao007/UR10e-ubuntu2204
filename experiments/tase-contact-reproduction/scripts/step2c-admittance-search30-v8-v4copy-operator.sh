...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2c_admittance_search30_guard20_search2ms_line1ms_alpha70_v8_v4copy.urp"
EXPECTED_BASENAME="step2c_admittance_search30_guard20_search2ms_line1ms_alpha70_v8_v4copy.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2c-admittance-search30-v8-v4copy-operator.sh bridge
  step2c-admittance-search30-v8-v4copy-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step2/step2c_admittance_search30_guard20_search2ms_line1ms_alpha70_v8_v4copy.urp
...
ax.set_title("Step2C v8 v4copy Fz Tracking")
...
while True:
    ...
    if not snap["loaded_ok"]:
        print(snap["raw"], flush=True)
        print("[autowatch] refusing: loaded program is not the expected v8 .urp", flush=True)
        raise SystemExit(6)

    if snap["running"]:
        print("[autowatch] expected v8 program is running; starting bridge.", flush=True)
        raise SystemExit(0)
...
case "${mode}" in
  autowatch)
    cat <<'WARNING'
STEP2C admittance-search30 v8 v4copy autowatch.
...
The bridge will start automatically only after Dashboard reports that exact v8 program running.
WARNING
    ...
    STAMP="$(date +%Y%m%d_%H%M%S)"
    out_dir="${RUN_ROOT}/bridge_step2c_v8_v4copy_autowatch_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
  ...
  bridge)
    cat <<'WARNING'
STEP2C admittance-search30 v8 v4copy lifecycle bridge.
...
Type START_STEP2C_ADMITTANCE30_V8_V4COPY to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP2C_ADMITTANCE30_V8_V4COPY" ]]; then
      echo "aborted"
      exit 2
    fi

    out_dir="${RUN_ROOT}/bridge_step2c_v8_v4copy_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
