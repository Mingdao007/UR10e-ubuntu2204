...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2c_admittance_search15_guard20_search2ms_line1ms_alpha70_v10_v9soft3.urp"
EXPECTED_BASENAME="step2c_admittance_search15_guard20_search2ms_line1ms_alpha70_v10_v9soft3.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2c-admittance-search15-v10-v9soft3-operator.sh bridge
  step2c-admittance-search15-v10-v9soft3-operator.sh autowatch

Teach Pendant program:
  /programs/andyl/kunwei/step2/step2c_admittance_search15_guard20_search2ms_line1ms_alpha70_v10_v9soft3.urp
...
Motion settings in the TP program:
  ...
  far search: 15 mm/s, 500 mm/s^2, speedl t=2 ms
  ...
  soft acquisition: 2 N velocity admittance, gain 0.0015, vlim 3 mm/s, speedl t=2 ms
...
ax.set_title("Step2C v10 v9soft3 Fz Tracking")
...
while True:
    ...
    if not snap["loaded_ok"]:
        print(snap["raw"], flush=True)
        print("[autowatch] refusing: loaded program is not the expected v10 .urp", flush=True)
        raise SystemExit(6)

    if snap["running"]:
        print("[autowatch] expected v10 program is running; starting bridge.", flush=True)
        raise SystemExit(0)
...
case "${mode}" in
  autowatch)
    cat <<'WARNING'
STEP2C admittance-search15 v10 v9soft3 autowatch.
...
The bridge will start automatically only after Dashboard reports that exact v10 program running.
WARNING
    ...
    STAMP="$(date +%Y%m%d_%H%M%S)"
    out_dir="${RUN_ROOT}/bridge_step2c_v10_v9soft3_autowatch_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
  ...
  bridge)
    cat <<'WARNING'
STEP2C admittance-search15 v10 v9soft3 lifecycle bridge.
...
Motion:
  far search = 15 mm/s, acceleration = 500 mm/s^2, speedl t = 2 ms
  near search = 5 mm/s, acceleration = 500 mm/s^2, speedl t = 2 ms
  soft acquisition = 2 N, gain = 0.0015, velocity limit = 3 mm/s, speedl t = 2 ms
...
Type START_STEP2C_ADMITTANCE15_V10_V9SOFT3 to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP2C_ADMITTANCE15_V10_V9SOFT3" ]]; then
      echo "aborted"
      exit 2
    fi

    out_dir="${RUN_ROOT}/bridge_step2c_v10_v9soft3_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
