...
EXPECTED_PROGRAM="/programs/andyl/kunwei/step2/step2c_admittance_search30_guard20_search2ms_line1ms_alpha70_v3.urp"
EXPECTED_BASENAME="step2c_admittance_search30_guard20_search2ms_line1ms_alpha70_v3.urp"
...
usage() {
  cat <<'USAGE'
Usage:
  step2c-admittance-search30-v3-operator.sh bridge

Teach Pendant program:
  /programs/andyl/kunwei/step2/step2c_admittance_search30_guard20_search2ms_line1ms_alpha70_v3.urp
...
case "${mode}" in
  bridge)
    cat <<'WARNING'
STEP2C admittance-search30 v3 lifecycle bridge.
...
Type START_STEP2C_ADMITTANCE30_V3 to continue:
WARNING
    read -r confirm
    if [[ "${confirm}" != "START_STEP2C_ADMITTANCE30_V3" ]]; then
      echo "aborted"
      exit 2
    fi

    out_dir="${RUN_ROOT}/bridge_step2c_admittance_search30_v3_search2ms_line1ms_alpha70_${STAMP}"
    mkdir -p "${out_dir}"
