...
if [[ "${STEP5D_VERSION}" == "step5d_strict_rnn_ablation_v27" ]]; then
  STEP5D_DEFAULT_MAX_NORMAL_FORCE_N="${STEP5D_DEFAULT_MAX_NORMAL_FORCE_N:-35}"
  STEP5D_DEFAULT_MAX_FORCE_NORM_N="${STEP5D_DEFAULT_MAX_FORCE_NORM_N:-35}"
...
if [[ "${STEP5D_VERSION}" == "step5d_strict_rnn_ablation_v25" ]]; then
  ...
  STEP5D_DEFAULT_REZERO_S="${STEP5D_DEFAULT_REZERO_S:-0.25}"
else
...
Boundary:
  ...
  - Raw normal guard: current v24/v25/v26 defaults to 25 N, v27 defaults to 35 N, torque guard 4.0 Nm; retained v18-v23 evidence packages used 100/100/4.0.
  ...
  - v27 default tube is filtered 5-22 N, raw-sanity 3-25 N, and force_norm <=35 N for 0.100 s before Stage 25.95 verifies registers 37..47 clear.
...
case "$1" in
  ...
  contact-bridge)
    ...
    BRIDGE_TARGET_FORCE_N="${BRIDGE_TARGET_FORCE_N:-${STEP5D_TARGET_FORCE_N:-12.0}}" \
    STEP4E_TARGET_FORCE_N="${STEP4E_TARGET_FORCE_N:-${STEP5D_TARGET_FORCE_N:-12.0}}" \
    BRIDGE_NORMAL_FOLLOW_MODE="${BRIDGE_NORMAL_FOLLOW_MODE:-${STEP5D_NORMAL_FOLLOW_MODE:-filtered_live}}" \
    BRIDGE_NORMAL_FILTER_ALPHA="${BRIDGE_NORMAL_FILTER_ALPHA:-${STEP5D_NORMAL_FILTER_ALPHA:-0.55}}" \
    BRIDGE_NORMAL_MIN_FORCE_N="${BRIDGE_NORMAL_MIN_FORCE_N:-${STEP5D_NORMAL_MIN_FORCE_N:-2.0}}" \
    BRIDGE_FORCE_P_GAIN="${BRIDGE_FORCE_P_GAIN:-${STEP5D_FORCE_P_GAIN:-0.0010}}" \
    BRIDGE_FORCE_I_GAIN="${BRIDGE_FORCE_I_GAIN:-${STEP5D_FORCE_I_GAIN:-0.00001}}" \
