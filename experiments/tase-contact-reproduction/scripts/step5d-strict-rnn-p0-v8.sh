...
PHASE_S must be exactly 2, 10, or 60. The 2/10 second phases are canary-only;
only a continuous verified 60 second artifact can mark P0 v8 passed.
...
case "${1:-}" in
  ...
  capture-ready|capture-bridge)
    ...
    export STEP5D_P0_PHASE_S="$(require_phase "$2")"
    exec "${BASE_WRAPPER}" "$1"
    ;;
  validate-run)
    if [[ $# -ne 3 ]]; then
