...
VERSION_DIR = Path("/Users/andyl/Documents/UR5e/eoat/eoat_print/v13_ksm8n_receiver_5p3mm_side_window_85mm")
...
def cut_receiver_features(shape, receiver_front_z: float, receiver_radius_mm: float):
    ...
    side_window_height = p["nut_side_window_height_mm"]
    ...
    side_slot = Part.makeBox(
        ...
        Vector(
            ...
            nut_center_z - side_window_height / 2.0,
        ),
    )
...
def cut_front_face_recess(shape, receiver_front_z: float):
    ...
    outer = make_cylinder_z(
        p["receiver_diameter_mm"] + 0.6,
        recess_depth + 0.3,
        receiver_front_z - recess_depth,
    )
    inner_boss_keepout = make_cylinder_z(
        p["front_face_seating_boss_diameter_mm"],
        recess_depth + 0.7,
        receiver_front_z - recess_depth - 0.2,
    )
    flat_bottom_recess = outer.cut(inner_boss_keepout)
    return shape.cut(flat_bottom_recess)
...
def write_readme(verification):
    p = PARAMS
    text = f"""# UR5e KSM-8N ball-transfer tool v13, 5.3 mm side nut window, 85 mm TCP

This version keeps the v11 flat-bottom recessed receiver face and the same
center KSM socket depth. The KSM-8N remains a purchased metal part; the printed
receiver keeps a captive M6 hex nut while reducing the side opening height from
about 6.2 mm to 5.3 mm for a user-measured 4.8 mm nut.
...
- Receiver front face: flat-bottom recess, {p["front_face_recess_depth_mm"]:.1f} mm deep, with {p["front_face_seating_boss_diameter_mm"]:.1f} mm central seating boss
- Side nut window height: {p["nut_side_window_height_mm"]:.1f} mm

## Output files

- `ur5e_ksm8n_ball_transfer_tool_v13_body.step / .stl`
- `ur5e_ksm8n_ball_transfer_tool_v13_ksm_placeholder.step / .stl`
- `ur5e_ksm8n_ball_transfer_tool_v13_assembly.step / .stl`
- `ur5e_ksm8n_ball_transfer_tool_v13_onepiece_preview.step / .stl`
- `ur5e_ksm8n_ball_transfer_tool_v13_receiver_fitcheck.step / .stl`
- `ur5e_ksm8n_ball_transfer_tool_v13_flange_fitcheck.step / .stl`
...
- The side window is 5.3 mm high for a measured 4.8 mm M6 nut, leaving smaller top/bottom lips around the opening without changing the center KSM socket depth.
...
def main():
    ...
    stem = "ur5e_ksm8n_ball_transfer_tool_v13"
    ...
    verification = {
        "version": "v13_ksm8n_receiver_5p3mm_side_window_85mm",
        "concept": "Short direct-flange UR5e passive rolling-contact EOAT using local KSM-8N M6 ball transfer with a slim receiver, unchanged center KSM socket depth, flat-bottom recessed front face, and 5.3 mm side nut window.",
        "parameters": PARAMS,
        "derived": {
            ...
            "front_face_recess_extends_to_receiver_od": True,
            "nut_side_window_z0_mm": positions["nut_center_z"] - PARAMS["nut_side_window_height_mm"] / 2.0,
            "nut_side_window_z1_mm": positions["nut_center_z"] + PARAMS["nut_side_window_height_mm"] / 2.0,
            "nut_side_window_height_mm": PARAMS["nut_side_window_height_mm"],
            "nut_measured_height_reference_mm": 4.8,
            "center_ksm_socket_depth_unchanged_from_v11": True,
        ...
        "validation": {
            ...
            "front_face_recess_is_flat_bottom_not_annular_rim": True,
            "side_nut_window_height_matches_user_request": True,
            "center_ksm_socket_depth_unchanged": True,
        },
        "assumptions_to_confirm": [
            ...
            "The 5.3 mm side nut window assumes the user's measured M6 nut height is about 4.8 mm; print receiver_fitcheck first to confirm insertion is not too tight.",
            "The receiver_fitcheck should be printed before the full body to confirm nut insertion, nut anti-rotation, KSM engagement, and glue clearance.",
